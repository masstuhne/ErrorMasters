...
function RecipeDisplay() {
    ...
    useEffect(()=>{
        ...
        .then(response =>{
            let savedIds= response.data.map(r=>r.id)
    ...
    const handleBookMarke = async (e)=>{
        e.preventDefault() 
        try{
            let tokenPayload=parseJwt(localStorage.getItem('user_ret'))
            let userId=tokenPayload.id
            const response= await axios.post("http://localhost:8080/api/v1/users/"+userId+"/bookmarked-recipes?recipeId="+id,JSON.stringify({
        
            }),{headers :{"Content-Type":"application/json", Authorization: `Bearer ${localStorage.getItem('user_ret')}`,} })
            ...
            <div className='px-36 justify-center p-4 relative w-full p-4 grid grid-cols-2'>
                <div>
                    ...
                    <a href='#' className='px-1 text-blue-700'>
                        {isBell ?  
                        <svg onClick={e=>setIsBell(false)} class="w-6 h-6 text-blue-700 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 25">
                        ...
                        :
                        <svg onClick={e=>setIsBell(true)} class="w-6 h-6 text-blue-700 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 25">
                    ...
                    <a href='#' className='px-1 text-blue-700'>
                        ...
                        :
                        <svg onClick={handleBookMarke} class="w-6 h-6 text-blue-700 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 25">
